...
def _extract_reservation_data(reservation):
    creator = f"{reservation.created_by.first_name} {reservation.created_by.last_name}".strip()
    creator = creator or getattr(reservation.created_by, "email", "") or "Usuario"

    approver = ""
    if reservation.approved_by:
        approver = f"{reservation.approved_by.first_name} {reservation.approved_by.last_name}".strip()
        approver = approver or getattr(reservation.approved_by, "email", "")

    return {
        "id": reservation.id,
        "title": reservation.title or "Sin título",
        "description": reservation.description or "",
        "space": getattr(reservation.space, "name", "Sin espacio"),
        "location": getattr(reservation.space, "location", ""),
